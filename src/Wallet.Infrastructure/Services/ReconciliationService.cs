    ...
    IClock clock) : IReconciliationService
{
    public async Task<ReconciliationReportResult> RunReconciliationAsync(DateOnly? watDate, string runBy, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var extLedgerAcct = await externalAccounts.GetByKeyForUpdateAsync(ExternalAccountKeys.LedgerHolding, ct)
                ?? throw new DomainException("external_account.missing", "Ledger holding account is not seeded.");
            var extSettlementAcct = await externalAccounts.GetByKeyForUpdateAsync(ExternalAccountKeys.SettlementHolding, ct)
                ?? throw new DomainException("external_account.missing", "Settlement holding account is not seeded.");

            var ledgerDiff = customerSum - extLedgerAcct.BalanceKobo;
            if (ledgerDiff > 0) extLedgerAcct.Credit(Money.Create(ledgerDiff), now);
            else if (ledgerDiff < 0) extLedgerAcct.Debit(Money.Create(-ledgerDiff), now);

            var settlementDiff = settlementSys - extSettlementAcct.BalanceKobo;
            if (settlementDiff > 0) extSettlementAcct.Credit(Money.Create(settlementDiff), now);
            else if (settlementDiff < 0) extSettlementAcct.Debit(Money.Create(-settlementDiff), now);

            await uow.SaveChangesAsync(ct);

            var extLedgerBal = extLedgerAcct.BalanceKobo;
            var extSettlementBal = extSettlementAcct.BalanceKobo;
